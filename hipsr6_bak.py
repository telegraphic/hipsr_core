...
class MbCalSpectrum(tb.IsDescription):
  """ PyTables table descriptor: storage of spectral data """
  id         = tb.Int32Col(pos=0)             # Unique ID
  timestamp  = tb.Time64Col(pos=1)            # Timestamp (at BRAM read)
  xx_cal_on  = tb.Int32Col(shape=16, pos=6)   # Noise diode ON, X pol
...
def createMultiBeamCal(filename, path='./', num_beams=13):
    """ Create a new HDF5 file and populate with table structure for noise diode cal
    ...
    for id in range(1,num_beams+1):
      ...
      h5.createTable(raw_data, 'beam_%02i'%id, MbCalSpectrum, 'Calibration for beam %02s'%id)
...
if __name__ == "__main__":
    ...
    x = createMultiBeamCal('test.h5', num_beams=13)
    print x
    x.close()
    os.remove('test.h5')